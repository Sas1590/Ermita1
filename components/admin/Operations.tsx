...
interface ContactMessage { id: string; name: string; email: string; phone: string; subject: string; message: string; timestamp: number; read: boolean; }
interface Reservation { id: string; name: string; phone: string; pax: string; notes: string; date: string; time: string; dateTimeIso: string; createdAt: number; status: 'pending' | 'confirmed' | 'cancelled'; privacy: boolean; }
interface BackupItem { id: string; timestamp: number; name: string; data: AppConfig; }

const BETA_DATA_DAILY = { title: "Menú Diari", subtitle: "DE DIMARTS A DIVENDRES", icon: "lunch_dining", visible: true, recommended: true, price: "18€", vat: "IVA inclòs", disclaimer: "Vàlid de dimarts a divendres (no festius)", sections: [ { title: "PRIMERS PLATS", items: [ { nameCa: "Amanida de formatge de cabra amb fruits secs", nameEs: "Ensalada de queso de cabra", visible: true }, { nameCa: "Canelons casolans de l'àvia", nameEs: "Canelones caseros de la abuela", visible: true }, { nameCa: "Escudella barrejada", nameEs: "Escudella catalana", visible: true } ] }, { title: "SEGONS PLATS", items: [ { nameCa: "Botifarra a la brasa amb mongetes", nameEs: "Butifarra a la brasa con judías", visible: true }, { nameCa: "Peix fresc de la llotja", nameEs: "Pescado fresco del día", visible: true }, { nameCa: "Pollastre de pagès rostit", nameEs: "Pollo de payés asado", visible: true } ] }, { title: "POSTRES", items: [ { nameCa: "Crema Catalana", nameEs: "Crema Catalana", visible: true }, { nameCa: "Flam d'ou casolà", nameEs: "Flan de huevo casero", visible: true }, { nameCa: "Fruita del temps", nameEs: "Fruta del tiempo", visible: true } ] } ], drinks: ["Aigua", "Vi de la casa", "Gasosa"], infoIntro: "El menú inclou primer plat, segon plat, postres, pa, aigua i vi.", infoAllergy: "Si tens alguna al·lèrgia, informa el nostre personal.", footerText: "Cuina de mercat" };
const BETA_DATA_CARTA = { title: "Carta de Menjar", subtitle: "", icon: "restaurant_menu", visible: true, recommended: false, price: "", vat: "", infoIntro: "", infoAllergy: "", showPrice: false, showInfo: false, showDisclaimer: true, disclaimer: "", sections: [ { id: "sec_tapas", category: "TAPES · TAPAS", icon: "tapas", items: [ { nameCa: "Gilda d'anxova de Perellò 1898", nameEs: "Anxova 00, oliva gordal, piparra de Navarra i tomàquet sec (1 unitat).", price: "3.50€", visible: true } ] } ] };
const BETA_DATA_WINE = { title: "Carta de Vins", subtitle: "", icon: "wine_bar", visible: true, recommended: false, price: "", vat: "", infoIntro: "", infoAllergy: "", showPrice: false, showInfo: false, showDisclaimer: true, disclaimer: "", categories: [ { category: "VINS NEGRES", icon: "wine_bar", groups: [ { sub: "D.O. TERRA ALTA", items: [ { name: "Llàgrimes de Tardor", desc: "Garnatxa, Carinyena", price: "18.50€", visible: true } ] } ] } ] };
const BETA_DATA_GROUP = { title: "Menú de Grup", subtitle: "MÍNIM 10 PERSONES", icon: "diversity_3", visible: true, recommended: false, price: "35.00€", vat: "IVA INCLÒS", disclaimer: "Mínim 10 persones. Cal reservar amb antelació.", sections: [ { title: "PICA-PICA (PER COMPARTIR)", icon: "tapas", items: [ { nameCa: "Assortiment d'embotits ibèrics i formatges", nameEs: "Surtido de embutidos ibéricos y quesos", visible: true } ] } ], drinks: ["Aigua mineral", "Vi de la casa (Negre/Blanc)", "Cafès"], infoIntro: "El menú inclou pa, aigua, vi i cafè.", infoAllergy: "Si us plau, consulteu els al·lèrgens al nostre personal." };

const MessageCard: React.FC<{ msg: ContactMessage; onToggleStatus: (id: string, currentStatus: boolean) => void; onDelete: (id: string) => void; }> = ({ msg, onToggleStatus, onDelete }) => {
        ...
        <div className={`rounded-xl border transition-all duration-300 group ${!msg.read ? 'bg-white border-blue-300 shadow-md border-l-[6px] border-l-blue-600' : 'bg-gray-50 border-gray-200 opacity-90 hover:opacity-100'}`}>
            <div className={`px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center border-b rounded-t-xl gap-4 ${!msg.read ? 'bg-blue-50/30 border-blue-100' : 'bg-gray-100/50 border-gray-200'}`}>
                <div className={`flex items-center gap-2 text-xs font-bold px-3 py-1 rounded-md border shrink-0 ${!msg.read ? 'text-blue-700 bg-blue-100/50 border-blue-200' : 'text-gray-500 bg-gray-200 border-gray-300'}`}>
                    ...
                    <span className="opacity-30 mx-1">|</span> <span>{new Date(msg.timestamp).toLocaleTimeString('ca-ES', { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
                <div className="flex items-center gap-3 flex-1 min-w-0">
                    {!msg.read ? (<div className="flex items-center gap-2 text-blue-600 bg-blue-50 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border border-blue-100"><span className="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse"></span>Nou</div>) : (<div className="flex items-center gap-2 text-gray-400 bg-gray-100 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border border-gray-200"><span className="material-symbols-outlined text-[10px]">done_all</span>Llegit</div>)}
...
export const Operations: React.FC<OperationsProps> = ({ activeTab, config, updateConfig, setLocalConfig, isSuperAdmin = false }) => {
    ...
    useEffect(() => {
        ...
        if (config?.adminSettings?.maxHeroImages !== undefined) setTempMaxHeroImages(config.adminSettings.maxHeroImages.toString());
    ...
    useEffect(() => {
        ...
        if (activeTab === 'inbox') {
            return onValue(ref(db, 'contactMessages'), (snapshot) => {
                if (snapshot.exists()) {
                    const data = snapshot.val();
        ...
        if (activeTab === 'security') {
            return onValue(ref(db, 'backups'), (snapshot) => {
                if (snapshot.exists()) {
                    const data = snapshot.val();
    ...
    const handleUpdateReservationStatus = async (resId: string, newStatus: 'confirmed' | 'cancelled' | 'pending') => { try { await update(ref(db, `reservations/${resId}`), { status: newStatus }); } catch (e) { console.error(e); } };
    const handleDeleteReservation = (resId: string) => { setConfirmModal({ isOpen: true, title: "Eliminar Reserva", message: "Segur que vols eliminar aquesta reserva?", type: 'danger', onConfirm: async () => { try { await remove(ref(db, `reservations/${resId}`)); } catch (e) { console.error(e); } setConfirmModal(prev => ({ ...prev, isOpen: false })); } }); };
    const handleToggleMessageReadStatus = async (messageId: string, currentStatus: boolean) => { try { await update(ref(db, `contactMessages/${messageId}`), { read: !currentStatus }); if (!currentStatus) showFeedback('success', "Missatge mogut a l'Històric."); else showFeedback('success', "Missatge retornat a Pendents."); } catch (e) { console.error(e); } };
    const handleDeleteMessage = (messageId: string) => { setConfirmModal({ isOpen: true, title: "Eliminar Missatge", message: "Estàs segur que vols esborrar aquest missatge?", type: 'danger', onConfirm: async () => { try { await remove(ref(db, `contactMessages/${messageId}`)); } catch (e) { console.error(e); } setConfirmModal(prev => ({ ...prev, isOpen: false })); } }); };
    
    const handleInitiateBackup = () => {
        if (!config) { setConfirmModal({ isOpen: true, title: "Error", message: "No s'ha pogut llegir la configuració actual.", type: 'danger', onConfirm: () => setConfirmModal(prev => ({ ...prev, isOpen: false })) }); return; }
    ...
    const handleConfirmCreateBackup = async () => {
        ...
        setShowBackupInput(false); setCreatingBackup(true);
        try { 
            const sanitizedConfig = JSON.parse(JSON.stringify(config));
            await push(ref(db, 'backups'), { timestamp: Date.now(), name: newBackupName, data: sanitizedConfig }); 
            showFeedback('success', `Còpia "${newBackupName}" creada correctament.`); 
        } catch (e: any) { 
            let errorMsg = "Error desconegut creant la còpia.";
            if (e.code === 'PERMISSION_DENIED') errorMsg = "Permís denegat. Comprova que tens permís d'escriptura a 'backups' a Firebase Rules.";
            else if (e.message && e.message.includes('undefined')) errorMsg = "Error de dades: La configuració conté camps invàlids.";
            setConfirmModal({ isOpen: true, title: "Error al crear còpia", message: errorMsg, type: 'danger', onConfirm: () => setConfirmModal(prev => ({ ...prev, isOpen: false })) });
        } finally { setCreatingBackup(false); }
    };

    const handleRestoreBackup = (backup: BackupItem) => { if (!updateConfig || !setLocalConfig) return; setConfirmModal({ isOpen: true, title: "Restaurar Còpia", message: `Estàs segur de restaurar la còpia "${backup.name}"?`, type: 'warning', onConfirm: async () => { try { await updateConfig(backup.data); setLocalConfig(backup.data); showFeedback('success', "Restauració completada."); } catch(e) { showFeedback('error', "Error restaurant."); } setConfirmModal(prev => ({ ...prev, isOpen: false })); } }); };
    const handleDeleteBackup = (backupId: string) => { setConfirmModal({ isOpen: true, title: "Eliminar Còpia", message: "Esborrar aquesta còpia de seguretat permanentment?", type: 'danger', onConfirm: async () => { try { await remove(ref(db, `backups/${backupId}`)); } catch(e) { console.error(e); } setConfirmModal(prev => ({ ...prev, isOpen: false })); } }); };

    const handleSetMasterVersion = async () => { if (!config) return; try { const sanitizedConfig = JSON.parse(JSON.stringify(config)); await set(ref(db, 'backups/master_delivery'), { timestamp: Date.now(), name: "Versió Entrega (Master)", data: sanitizedConfig }); showFeedback('success', "Versió d'Entrega guardada correctament."); setShowSetMasterConfirm(false); } catch (e) { console.error(e); showFeedback('error', "Error guardant la versió mestra."); } };
    const performFactoryReset = async () => { if (!updateConfig || !setLocalConfig) return; try { const snapshot = await get(ref(db, 'backups/master_delivery')); if (snapshot.exists()) { const masterData = snapshot.val(); if (masterData && masterData.data) { await updateConfig(masterData.data); setLocalConfig(masterData.data); showFeedback('success', "Restaurada Versió Inicial (Entrega)."); } else { throw new Error("Master data corrupt"); } } else { await updateConfig(defaultAppConfig); setLocalConfig(defaultAppConfig); showFeedback('success', "Restaurat a valors per defecte (Codi)."); } setShowFactoryResetConfirm(false); } catch (e) { console.error(e); try { await updateConfig(defaultAppConfig); setLocalConfig(defaultAppConfig); showFeedback('success', "Restaurat a valors per defecte (Fallback)."); } catch (err) { showFeedback('error', "Error crític al restaurar."); } setShowFactoryResetConfirm(false); } };

    const handleSaveLimits = async () => {
        const menusNum = parseInt(tempMaxMenus, 10); const heroNum = parseInt(tempMaxHeroImages, 10); const prodNum = parseInt(tempMaxProduct, 10); const histNum = parseInt(tempMaxHistoric, 10);
        ...
        return (
            <div className="space-y-8 animate-[fadeIn_0.3s_ease-out] max-w-4xl mx-auto relative">
                ...
                <div className="bg-white p-6 rounded shadow-sm border border-gray-200">
                    ...
                    <div className="flex justify-between items-center mb-6"><h4 className="font-bold text-gray-700">Historial</h4><button onClick={handleInitiateBackup} disabled={creatingBackup} className={`bg-primary hover:bg-accent text-white px-4 py-2 rounded text-xs font-bold uppercase flex items-center gap-2 shadow-md transition-all ${creatingBackup ? 'opacity-70 cursor-wait' : ''}`}>{creatingBackup ? (<><span className="w-3 h-3 border-2 border-white/50 border-t-white rounded-full animate-spin"></span> Guardant...</>) : (<><span className="material-symbols-outlined text-sm">save</span> Crear còpia</>)}</button></div>
                ...
                {renderConfirmModal()}
                {showBackupInput && (<div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]"><div className="bg-white rounded-lg shadow-2xl p-8 max-w-sm w-full border-t-4 border-primary text-center"><div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4 text-primary"><span className="material-symbols-outlined text-3xl">save</span></div><h3 className="font-serif text-xl font-bold text-gray-800 mb-2">Nom de la Còpia</h3><p className="text-gray-500 mb-4 text-xs">Tria un nom per identificar aquest punt de restauració.</p><input type="text" value={newBackupName} onChange={(e) => setNewBackupName(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm outline-none focus:border-primary mb-6 text-center font-bold" placeholder="Ex: Còpia Dimarts" autoFocus /><div className="flex gap-3"><button onClick={() => setShowBackupInput(false)} className="flex-1 py-3 border border-gray-300 rounded text-gray-600 font-bold uppercase text-xs hover:bg-gray-50 transition-colors">Cancel·lar</button><button onClick={handleConfirmCreateBackup} className="flex-1 py-3 bg-primary text-white rounded font-bold uppercase text-xs hover:bg-accent shadow-md transition-colors">Guardar</button></div></div></div>)}
    ...
    if (activeTab === 'superadmin' && isSuperAdmin) {
        return (
            <div className="space-y-8 animate-[fadeIn_0.3s_ease-out] max-w-4xl mx-auto relative">
                ...
                {renderConfirmModal()}
                {showFactoryResetConfirm && (<div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]"><div className="bg-white rounded-lg shadow-2xl p-8 max-w-sm w-full border-t-4 border-red-600 text-center"><div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4 text-red-600"><span className="material-symbols-outlined text-3xl">history</span></div><h3 className="font-serif text-2xl font-bold text-gray-800 mb-2">Restaurar Versió Entrega?</h3><p className="text-gray-500 mb-6 text-sm leading-relaxed"><strong className="text-red-600 block mb-2">ATENCIÓ: Això esborrarà TOTA la configuració actual.</strong> La web tornarà a estar exactament com el dia que es va entregar al client.</p><div className="flex gap-3"><button onClick={() => setShowFactoryResetConfirm(false)} className="flex-1 py-3 border border-gray-300 rounded text-gray-600 font-bold uppercase text-xs hover:bg-gray-50 transition-colors">Cancel·lar</button><button onClick={performFactoryReset} className="flex-1 py-3 bg-red-600 text-white rounded font-bold uppercase text-xs hover:bg-red-700 shadow-md transition-colors">Sí, Restaurar</button></div></div></div>)}
                {showSetMasterConfirm && (<div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]"><div className="bg-white rounded-lg shadow-2xl p-8 max-w-sm w-full border-t-4 border-indigo-600 text-center"><div className="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center mx-auto mb-4 text-indigo-600"><span className="material-symbols-outlined text-3xl">save_as</span></div><h3 className="font-serif text-2xl font-bold text-gray-800 mb-2">Definir Versió Mestra?</h3><p className="text-gray-500 mb-6 text-sm leading-relaxed">Estàs a punt de guardar l'estat actual com a <strong>Punt de Restauració d'Entrega</strong>. <br/><br/><span className="italic text-xs">Si en el futur es prem "Restaurar Versió Inicial", es carregarà exactament el que veus ara a la web.</span></p><div className="flex gap-3"><button onClick={() => setShowSetMasterConfirm(false)} className="flex-1 py-3 border border-gray-300 rounded text-gray-600 font-bold uppercase text-xs hover:bg-gray-50 transition-colors">Cancel·lar</button><button onClick={handleSetMasterVersion} className="flex-1 py-3 bg-indigo-600 text-white rounded font-bold uppercase text-xs hover:bg-indigo-700 shadow-md transition-colors">Confirmar i Guardar</button></div></div></div>)}
            </div>
        );
    }
    return null;
