...
const Menu: React.FC<MenuProps> = ({ activeTab, onToggleTab }) => {
  ...
  const getCardClasses = (isRecommended: boolean, isActive: boolean) => {
      ...
      if (isRecommended) containerClass += "border-2 border-primary/30 z-10 scale-[1.01] "; else containerClass += "border border-white/10 opacity-95 hover:opacity-100 ";
      let headerClass = "w-full text-left px-8 py-8 flex justify-between items-center transition-colors duration-300 ";
      if (isActive) headerClass += "bg-[#1a1816] text-primary "; else headerClass += isRecommended ? "bg-white hover:bg-[#f0ece6] text-[#2c241b] " : "bg-[#e8e4d9] hover:bg-[#dcd6c8] text-[#2c241b] ";
  ...
  const toggleTab = (tab: string) => { if (activeTab === tab) onToggleTab(null); else { onToggleTab(tab); setTimeout(() => { const section = document.getElementById('carta'); if (section) { const offset = 100; const elementPosition = section.getBoundingClientRect().top + window.scrollY; const offsetPosition = elementPosition - offset; window.scrollTo({ top: offsetPosition, behavior: 'smooth' }); } }, 100); } };
